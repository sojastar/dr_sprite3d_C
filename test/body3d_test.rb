...
describe Engine3D::Body do
  it 'initializes' do
    body = Engine3D::Body.new do
      add_sprite Engine3D::Sprite3D.new(1.0, 2.0, 3.0, 'sprite_file.png', 0, 0, 16, 16)
      add_sprite Engine3D::Sprite3D.new(4.0, 5.0, 6.0, 'sprite_file.png', 0, 0, 16, 16)
      add_sprite Engine3D::Sprite3D.new(7.0, 8.0, 9.0, 'sprite_file.png', 0, 0, 16, 16)
    end

    assert_equal Engine3D::Matrix.identity, body.world_matrix
    assert_equal 3, body.sprites.length
  end
  ...
  it 'can store vertices' do
    ...
    assert_equal 3, body.sprites.length
    assert_equal Engine3D::Vector.new(1.0, 2.0, 3.0, 1.0), body.sprites[0].local
    assert_equal Engine3D::Vector.new(4.0, 5.0, 6.0, 1.0), body.sprites[1].local
    assert_equal Engine3D::Vector.new(7.0, 8.0, 9.0, 1.0), body.sprites[2].local
  end

  it 'directly moves to' do
    body = Engine3D::Body.new { add_sprite Engine3D::Sprite3D.new(1.0, 2.0, 3.0, 'sprite_file.png', 0, 0, 16, 16) }
